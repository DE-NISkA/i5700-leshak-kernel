...
{
	struct pmem_data      *data;
	struct pmem_mem_alloc param;
	int id = get_id(file);

	switch (cmd) {
	case PMEM_CACHE_INVALID:
		{
			if(copy_from_user(&param, (struct pmem_mem_alloc *)arg, sizeof(struct pmem_mem_alloc))){
